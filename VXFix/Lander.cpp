...
double current_vx() {
  ...
  if((current_velocityX - past_velocityX) > 1){
    fail = true;
  }
  past_velocityX = current_velocityX;
  if(!fail) {
    current_velocityX = Velocity_X();
  } else {
    if(Position_X() > PLAT_X) {
      current_velocityX = -(past_velocityX + past_a * past_T);
    } else {
      current_velocityX = (past_velocityX + past_a * past_T);
    }
  }
  ...
  sc = (current_position - past_position);
  vc = (current_velocityX - past_velocityX);
  past_T = current_T;
  current_T = fabs(sc / vc);
  past_a = current_a;
  current_a = 2 * (sc - past_velocityX * current_T) / (current_T * current_T);
  if(Position_X() > PLAT_X) {
    cv = -(past_velocityX + current_a * current_T);
  } else {
    cv = (past_velocityX + current_a * current_T);
  }
  cout << "current v: " << current_velocityX << endl;
  cout << "current_T: " << current_T << endl;
  cout << "current a:" << current_a << endl;
  cout << "past v:" << past_velocityX << endl;
  cout << "past t:" << past_T << endl;
  cout << "past a: " << past_a << endl; 
  cout << "Ax: " << current_a << endl;
  cout << "sc: " << sc << endl;
  cout << "vc: " << vc << endl;
  cout << "Ev: " << cv << endl;
